...
<%System.out.print("能不能出来了");%>
			...
			var biaoshi = (leixing1.value==undefined||leixing1.value=="")?"":"&leixing1="+window.encodeURI(window.encodeURI(leixing1.value));
			var title = (leixing2.value==undefined||leixing2.value=="")?"":"&leixing2="+window.encodeURI(window.encodeURI(leixing2.value));
			var difang = (leixing3.value==undefined||leixing3.value=="")?"":"&leixing3="+window.encodeURI(window.encodeURI(leixing3.value));
			...
			<form action="<%=path%>/NoticeSearchServlet" method="post" >
				<table>
					...
					<td>
							...
							<input type="text" name="leixing1" id="leixing1" value="<%=request.getAttribute("leixing1") == null ? "":java.net.URLDecoder.decode((String)request.getAttribute("leixing1"),"UTF-8") %>">
						</td>
						<td>
							<font
								style="font-family: 微软雅黑; color: #333333;">题目：</font>
							<input type="text" name="leixing2" id="leixing2" value="<%=request.getAttribute("leixing2") == null ? "":java.net.URLDecoder.decode((String)request.getAttribute("leixing2"),"UTF-8" )%>">
						</td>
						<td>
							<font
								style="font-family: 微软雅黑; color: #333333;">关键词：</font>
							<input type="text" name="leixing3" id="leixing3" value="<%=request.getAttribute("leixing3") == null ? "":java.net.URLDecoder.decode((String)request.getAttribute("leixing3"),"UTF-8")%>">
						</td>
						<td>
				...
				<td align="center" valign="middle">
					...
					<%
						...
						%><tr>
							...
							<td>
								<a
									href="<%=path%>/NoticeSelOneServlet?id=<%=notice.get("ID")%>&state=1">编辑</a>|
								<a href="javascript:void(0)" id="<%=notice.get("ID")%>" onclick="deleteDialog(this.id)">删除</a>|
								<a	href="<%=path%>/NoticeSelOneServlet?id=<%=notice.get("ID")%>&state=0">查看</a>
							</td>
						</tr>
						<%
							}
