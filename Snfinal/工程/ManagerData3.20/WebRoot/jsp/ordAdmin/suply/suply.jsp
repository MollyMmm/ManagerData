...
PageBean pageBean = (PageBean) request.getAttribute("suply");
...
<html>
  <head>
    ...
    <title>My JSP 'suply.jsp' starting page</title>
...
switch(id)
			...
			location = "<%=path%>/SuplyServlet?currentPage="+currentPage.value;
...
function Other(){
	...
	location = "<%=path%>/SuplyServlet?currentPage="+goPage;
	}	
	}
<% 	
break;
...
<%System.out.print("能不能出来了");%>
		...
		var difang = (leixing3.value==undefined||leixing3.value=="")?"":"&leixing3="+leixing3.value;
			location = "<%=path%>SuplySearchServlet?currentPage="+currentPage.value+biaoshi+title+difang;
...
function Other(){
	...
	if(goPage>=1&&goPage<=<%=pageBean.getTotalPage()%>){
		...
		var title = (leixing2.value==undefined||leixing2.value=="")?"":"&leixing2="+leixing2.value;
		var difang = (leixing3.value==undefined||leixing3.value=="")?"":"&leixing3="+leixing3.value;
		location = "<%=path%>SuplySearchServlet?currentPage="+goPage+biaoshi+title+difang;
	}		
}
<%
break;
...
<%--添加信息--%>
		function suplyAdd(id){
			 art.dialog.open('<%=path%>/jsp/ordAdmin/suply/addSuply.jsp', {
				title: '添加供求信息',
				time:1200,
				 //遮罩层
			    lock:true,
	...
	<body>
		<div style="margin-right: 10px; width: 850px">
			<form action="<%=path%>/SuplySearchServlet" method="post" >
				<table>
					<tr>
					<td>
						...
						<td>
							...
							<input type="button" value="创建" onclick="suplyAdd(this.id);"  id="suply">
					...
					<%
						...
						<%
							for (int i = 0; i < list.size(); i++) {
								Map<String, Object> suply = list.get(i);
		System.out.println("--------------------i------"+i);
								String body = String.valueOf(suply.get("INTRODUCTION"));
								System.out.println("--------------------i------"+body);
						%><tr>

							<td><%=suply.get("ID")%></td>
							<td><%=suply.get("EXTENDEDID")%></td>
							<td><%=suply.get("NAMES")%></td>
							<td><%=suply.get("PICTURE")%></td>
							<td><%=suply.get("PRICE")%></td>
							<td><%=suply.get("PREPRICE")%></td>
							<td><%if(body.length()>10){%><%=body.substring(0,9)+"..."%><%}else{%><%=body%><%}%></td>
							
							<td>
								<a
									href="<%=path%>/SuplySelOneServlet?id=<%=suply.get("ID")%>&state=1">编辑</a>|
								<a href="javascript:void(0)" id="<%=suply.get("ID")%>" onclick="deleteDialog(this.id)">删除</a>|
								<a	href="<%=path%>/SuplySelOneServlet?id=<%=suply.get("ID")%>&state=0">查看</a>
							</td>
						</tr>
						<%
							}
