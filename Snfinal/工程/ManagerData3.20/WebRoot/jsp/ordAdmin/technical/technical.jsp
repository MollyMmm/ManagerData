...
PageBean pageBean = (PageBean) request.getAttribute("technical");
...
<html>
  <head>
    ...
    <title>My JSP 'technical.jsp' starting page</title>
...
switch(id)
			...
			location = "<%=path%>/TechnicalServlet?currentPage="+currentPage.value;
...
function Other(){
	...
	location = "<%=path%>/TechnicalServlet?currentPage="+goPage;
	}	
	}
<% 	
break;
...
<%System.out.print("能不能出来了");%>
		...
		var difang = (leixing3.value==undefined||leixing3.value=="")?"":"&leixing3="+leixing3.value;
			location = "<%=path%>/TechnicalSearchServlet?currentPage="+currentPage.value+biaoshi+title+difang;
...
function Other(){
	...
	if(goPage>=1&&goPage<=<%=pageBean.getTotalPage()%>){
		...
		var title = (leixing2.value==undefined||leixing2.value=="")?"":"&leixing2="+leixing2.value;
		var difang = (leixing3.value==undefined||leixing3.value=="")?"":"&leixing3="+leixing3.value;
		location = "<%=path%>/TechnicalSearchServlet?currentPage="+goPage+biaoshi+title+difang;
	}		
}
<%
break;
...
<%--添加科技信息--%>
		function technicalAdd(id){
			 art.dialog.open('<%=path%>/jsp/ordAdmin/technical/addTechnical.jsp', {
				title: '添加科技信息',
				time:1200,
				 //遮罩层
			    lock:true,
	...
	<body>
		<div style="margin-right: 10px; width: 850px">
			<form action="<%=path%>/TechnicalSearchServlet" method="post" >
				<table>
					<tr>
					<td>
						...
						<td>
							...
							<input type="button" value="创建" onclick="technicalAdd(this.id);"  id="technical">
					...
					<%
						...
						<%
							for (int i = 0; i < list.size(); i++) {
								Map<String, Object> technical = list.get(i);
		System.out.println("--------------------i------"+i);
								String body = String.valueOf(technical.get("BODY"));
								System.out.println("--------------------i------"+body);
						%><tr>

							<td><%=technical.get("ID")%></td>
							<td><%=technical.get("EXTENDEDID")%></td>
							<td><%=technical.get("NAME")%></td>
							<td><%=technical.get("TITLE")%></td>
							<td><%if(body.length()>10){%><%=body.substring(0,9)+"..."%><%}else{%><%=body%><%}%></td>
							<td><%=technical.get("PUBLISHTIME")%></td>
							<td><%=technical.get("TYPE")%></td>
							<td>
								<a
									href="<%=path%>/TechnicalSelOneServlet?id=<%=technical.get("ID")%>&state=1">编辑</a>|
								<a href="javascript:void(0)" id="<%=technical.get("ID")%>" onclick="deleteDialog(this.id)">删除</a>|
								<a	href="<%=path%>/TechnicalSelOneServlet?id=<%=technical.get("ID")%>&state=0">查看</a>
							</td>
						</tr>
						<%
							}
