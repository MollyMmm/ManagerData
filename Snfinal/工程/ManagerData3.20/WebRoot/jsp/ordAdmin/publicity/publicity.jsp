...
<%System.out.print("能不能出来了");%>
			...
			location = "<%=path%>/PublicitySearchServlet?currentPage="+currentPage.value+biaoshi+title+difang;
...
function Other(){
	...
	if(goPage>=1&&goPage<=<%=pageBean.getTotalPage()%>){
		...
		var title = (leixing2.value==undefined||leixing2.value=="")?"":"&leixing2="+leixing2.value;
		var difang = (leixing3.value==undefined||leixing3.value=="")?"":"&leixing3="+leixing3.value;
		location = "<%=path%>/PublicitySearchServlet?currentPage="+goPage+biaoshi+title+difang;
	}		
}
<%
break;
...
<%--添加信息--%>
		function publicityAdd(){
			 art.dialog.open('<%=path%>/jsp/ordAdmin/publicity/addPublicity.jsp', {
				title: '添加政策法规',
				time:1200,
				 //遮罩层
			    lock:true,
				...
				<table>
					<tr>
						...
						<input type="hidden"  name="search" value="1"/>
							...
							<input type="text" name="leixing1"  value="<%=request.getParameter("leixing1") == null ? "":request.getParameter("leixing1") %>"  id="leixing1">
						</td>
						<td>
							<font
								style="font-family: 微软雅黑; color: #333333;" >内容：</font>
							<input type="text" name="leixing2" value="<%=request.getParameter("leixing2") == null ? "":request.getParameter("leixing2") %>" id="leixing2">
						</td>
						<td>
							<font
								style="font-family: 微软雅黑; color: #333333;">过程：</font>
							<input type="text" name="leixing3" value="<%=request.getParameter("leixing3") == null ? "":request.getParameter("leixing3") %>" id="leixing3">
						</td>
						<td>
							<input type="submit" value="搜索">
							<input
									type="button" value="创建" onclick="publicityAdd();"> 
						</td>
					</tr>
				</table>
			</form>
					...
					<%
						...
						<%
							for (int i = 0; i < list.size(); i++) {
								...
								String body = String.valueOf(Publicity.get("BODYS"));
						%><tr>
							...
							<td>
								<a
									href="<%=path%>/PublicitySelOneServlet?id=<%=Publicity.get("ID")%>&state=1">编辑</a>|
								<a href="javascript:void(0)" id="<%=Publicity.get("ID")%>" onclick="deleteDialog(this.id)"
									>删除</a>|
								<a	href="<%=path%>/PublicitySelOneServlet?id=<%=Publicity.get("ID")%>&state=0">查看</a>
							</td>
						</tr>
						<%
							}
