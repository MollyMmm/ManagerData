...
PageBean pageBean = (PageBean) request.getAttribute("news");
...
switch(id)
			...
			location = "<%=path%>/NewsServlet?currentPage="+currentPage.value;
...
function Other(){
	...
	location = "<%=path%>/NewsServlet?currentPage="+goPage;
	}	
	}
<% 	
break;
...
<%System.out.print("能不能出来了");%>
		...
		var difang = (leixing3.value==undefined||leixing3.value=="")?"":"&leixing3="+leixing3.value;
			location = "<%=path%>/NewsSearchServlet?currentPage="+currentPage.value+biaoshi+title+difang;
...
function Other(){
	...
	if(goPage>=1&&goPage<=<%=pageBean.getTotalPage()%>){
		...
		var title = (leixing2.value==undefined||leixing2.value=="")?"":"&leixing2="+leixing2.value;
		var difang = (leixing3.value==undefined||leixing3.value=="")?"":"&leixing3="+leixing3.value;
		location = "<%=path%>/NewsSearchServlet?currentPage="+goPage+biaoshi+title+difang;
	}		
}
<%
break;
...
<%--添加信息--%>
		function newsAdd(id){
			 art.dialog.open('<%=path%>/jsp/ordAdmin/news/addNews.jsp', {
				title: '添加时事新闻',
				time:1200,
				 //遮罩层
			    lock:true,
	...
	<body>
		<div style="margin-right: 10px; width: 850px">
			<form action="<%=path%>/NewsSearchServlet" method="post" >
				<table>
					<tr>
					<td>
						...
						<td>
							...
							<input type="button" value="创建" onclick="newsAdd(this.id);"  id="news">
					...
					<%
						...
						<%
							for (int i = 0; i < list.size(); i++) {
								Map<String, Object> news = list.get(i);
		System.out.println("--------------------i------"+i);
								String body = String.valueOf(news.get("BODY"));
								String mainBody = String.valueOf(news.get("MAINBODY"));
								System.out.println("--------------------i------"+body);
						%><tr>

							<td><%=news.get("ID")%></td>
							<td><%=news.get("EXTENDEDID")%></td>
							<td><%=news.get("TITLE")%></td>
							<td><%if(body.length()>10){%><%=body.substring(0,9)+"..."%><%}else{%><%=body%><%}%></td>
							<td><%if(mainBody.length()>10){%><%=mainBody.substring(0,9)+"..."%><%}else{%><%=mainBody%><%}%></td>
							<td><%=news.get("PUBLISH_TIME")%></td>
							<td><%=news.get("UPDATE_TIME")%></td>
							<td>
								<a
									href="<%=path%>/NewsSelOneServlet?id=<%=news.get("ID")%>&state=1">编辑</a>|
								<a href="javascript:void(0)" id="<%=news.get("ID")%>" onclick="deleteDialog(this.id)">删除</a>|
								<a	href="<%=path%>/NewsSelOneServlet?id=<%=news.get("ID")%>&state=0">查看</a>
							</td>
						</tr>
						<%
							}
