...
<%
	...
	String basePath = request.getScheme() + "://"
			...
			+ path + "/";
	PageBean pageBean = (PageBean) request.getAttribute("sellingsoil");
...
switch(id)
			...
			location = "<%=path%>/SellingSoilServlet?currentPage="+currentPage.value;
...
function Other(){
	...
	location = "<%=path%>/SellingSoilServlet?currentPage="+goPage;
	}
}		
function altRows(id) {
...
function deleteDialog(id){
	...
	throughBox({
		...
		button: [
        {
            name: '确定',
            callback: function () {
               window.location.href = "<%=path%>/SellingSoilDelServlet?ID='+id";
            },
            focus: true
        },
        {
	...
	<body>
		<div style="margin-right: 10px; width: 850px">
			<form action="<%=path%>/SellingSoilSearchServlet" method="post">
				...
				<table>
					...
					<td>
						...
						<td>
							<font
								style="font-family: 微软雅黑; color: #333333;">交易地点：</font>
							<input type="text" name="leixing3">
						</td>
						<td>
							<input type="submit" value="搜索">
							<a href="<%=path%>/jsp/ordAdmin/sellingSoil/addSellingSoil.jsp"><input
				...
				<td align="center" valign="middle">
					...
					<%
							if (list.size() == 0) {
								...
								<%if(request.getAttribute("leixing3") != null) {%>交易地点：<%=request.getAttribute("leixing3")%><%}%>
								信息不存在，如需创建，请点击创建按钮！
								</td>
								</tr>
						...
						<%
							for (int i = 0; i < list.size(); i++) {
								Map<String, Object> sellingsoil = list.get(i);
						%><tr>

							<td><%=sellingsoil.get("ID")%></td>
							<td><%=sellingsoil.get("EXTENDEDID")%></td>
							<td><%=sellingsoil.get("TITLE")%></td>
							<td><%=sellingsoil.get("TRADELOCATION")%></td>
							<td><%=sellingsoil.get("PUBLISHTIME")%></td>
							<td><%=sellingsoil.get("UPDATETIME")%></td>
							<td><%=sellingsoil.get("SALERID")%></td>
							<td><%=sellingsoil.get("SALERNAME")%></td>
							<td><%=sellingsoil.get("CONTACTMETHOD")%></td>
							<td><%=sellingsoil.get("VALIDDATE")%></td>
							<td><%=sellingsoil.get("ISVALID")%></td>
							<td><%=sellingsoil.get("SOILMESSAGE")%></td>
							<td>
								<a
									href="<%=path%>/SellingSoilSelOneServlet?id=<%=sellingsoil.get("ID")%>">编辑</a>|
								<a href="javascript:void(0)" id="<%=sellingsoil.get("ID")%>" onclick="deleteDialog(this.id)"
									>删除</a>
							</td>
						</tr>
						<%
