...
function deleteDialog(id){
	...
	throughBox({
		...
		button: [
        {
            name: '确定',
            callback: function () {
               window.location.href = "<%=path%>/PrivateConsultDeleteServlet?ID='+id";
            },
            focus: true
        },
        {
				...
				<td align="center" valign="middle">
					...
					<%
						...
						%><tr>
							...
							<td><%=consult.get("CONSULTING_BODY")%></td>
							<td><%=consult.get("CONSULTING_TIME")%></td>
							<td><%=consult.get("DEADLINE")%></td>
							<td><%=consult.get("CONSULTING_TYPE")%></td>
							<td><%=consult.get("REPLY_BODY")%></td>
							<td><%=consult.get("REPLY_TIME")%></td>
							<td>
								<a
									href="<%=path%>/SellingSoilSelOneServlet?id=<%=consult.get("ID")%>">编辑</a>|
								<a href="javascript:void(0)" id="<%=consult.get("ID")%>" onclick="deleteDialog(this.id)">删除</a>
							</td>
						</tr>
						<%
							}
