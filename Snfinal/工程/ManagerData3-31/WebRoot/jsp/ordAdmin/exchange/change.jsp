...
switch(id)
			...
			location = "<%=path%>/ExchangeChangeShowServlet?currentPage="+currentPage.value;
...
function Other(){
	...
	location = "<%=path%>/ExchangeChangeShowServlet?currentPage="+goPage;
	}
}		
function altRows(id) {
...
<tr>
						...
						%><tr>
							...
							<td><%=charge.get("C_ID")%></td>
							
							<td><%=charge.get("C_CHANGETYPE")%></td>
							<td><%=charge.get("P_ID")%></td>
							<td><%=charge.get("C_AMOUNT")%></td>
							<td><%=charge.get("C_ADNAME")%></td>
						</tr>
						<%
							}
						%>
