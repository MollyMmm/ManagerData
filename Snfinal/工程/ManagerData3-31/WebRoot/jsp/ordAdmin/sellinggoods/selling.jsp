...
PageBean pageBean = (PageBean) request.getAttribute("sellingGoods");
...
<html>
  <head>
    ...
    <title>My JSP 'selling.jsp' starting page</title>
...
switch(id)
			...
			location = "<%=path%>/SellingServlet?currentPage="+currentPage.value;
...
function Other(){
	...
	location = "<%=path%>/SellingServlet?currentPage="+goPage;
	}	
	}
<% 	
break;
...
<%System.out.print("能不能出来了");%>
			...
			location = "<%=path%>SellingSearchServlet?currentPage="+currentPage.value+biaoshi+title+difang;
...
function Other(){
	...
	if(goPage>=1&&goPage<=<%=pageBean.getTotalPage()%>){
		var biaoshi = (leixing1.value==undefined||leixing1.value=="")?"":"&leixing1="+leixing1.value;
		var title = (leixing1.value==undefined||leixing2.value=="")?"":"&leixing2="+leixing2.value;
		var difang = (leixing1.value==undefined||leixing2.value=="")?"":"&leixing3="+leixing3.value;
		location = "<%=path%>SellingSearchServlet?currentPage="+goPage+biaoshi+title+difang;
	}		
}
<%
break;
...
<%--添加信息--%>
		function sellingGoodsAdd(id){
			 art.dialog.open('<%=path%>/jsp/ordAdmin/sellinggoods/addSelling.jsp', {
				title: '添加农资采购信息',
				time:1200,
				 //遮罩层
			    lock:true,
	...
	<body>
		<div style="margin-right: 10px; width: 850px">
			<form action="<%=path%>/SellingSearchServlet" method="post" >
				<table>
					<tr>
					<td>
						...
						<td>
							<font
								style="font-family: 微软雅黑; color: #333333;">商品信息：</font>
							<input type="text" name="leixing3" id="leixing3" value="<%=request.getParameter("leixing3") == null ? "":request.getParameter("leixing3") %>">
						</td>
						<td>
							<input type="submit" value="搜索">
							<input type="button" value="创建" onclick="sellingGoodsAdd(this.id);"  id="sellingGoods">
				...
				<td align="center" valign="middle">
					...
					<%
							if (list.size() == 0) {
								...
								<%if(request.getAttribute("leixing3") != null) {%>商品信息：<%=request.getAttribute("leixing3")%><%}%>
								信息不存在，如需创建，请点击创建按钮！
								</td>
								</tr>
					...
					<th>题目</th>
						...
						<%
							for (int i = 0; i < list.size(); i++) {
								Map<String, Object> sellingGoods = list.get(i);
								String PRODUCT_MESSAGE = String.valueOf(sellingGoods.get("PRODUCT_MESSAGE"));
								String TRADE_LOCATION = String.valueOf(sellingGoods.get("TRADE_LOCATION"));
								String CONTACT_METHOD = String.valueOf(sellingGoods.get("CONTACT_METHOD"));
						%><tr>

							<td><%=sellingGoods.get("ID")%></td>
							<td><%=sellingGoods.get("EXTENDEDID")%></td>
							
							<td><%=sellingGoods.get("SALER_ID")%></td>
							<td><%=sellingGoods.get("SALER_NAME")%></td>
							<td><%=sellingGoods.get("TITLE")%></td>
							...
							<td><%=sellingGoods.get("PUBLISH_TIME")%></td>
							<td><%=sellingGoods.get("UPDATE_TIME")%></td>
							<td><%if(CONTACT_METHOD.length()>10){%><%=CONTACT_METHOD.substring(0,9)+"..."%><%}else{%><%=CONTACT_METHOD%><%}%></td>
							<td><%=sellingGoods.get("VALID_DATE")%></td>
							<td><%=sellingGoods.get("ISVALID")%></td>
							<td>
								<a
									href="<%=path%>/SellingSelOneServlet?id=<%=sellingGoods.get("ID")%>&state=1">编辑</a>|
								<a href="javascript:void(0)" id="<%=sellingGoods.get("ID")%>" onclick="deleteDialog(this.id)">删除</a>|
								<a	href="<%=path%>/SellingSelOneServlet?id=<%=sellingGoods.get("ID")%>&state=0">查看</a>
							</td>
						</tr>
						<%
							}
