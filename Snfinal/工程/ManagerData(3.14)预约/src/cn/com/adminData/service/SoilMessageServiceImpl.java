...
public class SoilMessageServiceImpl implements SoilMessageService{
	...
	public Boolean addSoilMessage(SoilMessageBean message, HttpSession session) {
		...
		String sql = "insert into SoilMessage(id,ExtendedId,Names,Price,Conversion_Property,SoilArea," +
		"Negotiable_Year,SurplusYear,SoilUsing,Land_Warrants,Type) values(SoilMessage_seq.nextval,"+ExtendedId+",'"+Names+"',"+Price+",'"+ConversionProperty+"','"+SoilArea+"','"+NegotiableYear+"','"+SurplusYear+"','"+SoilUsing+"','"+LandWarrants+"','"+Type+"')";
		int operate = 1;
