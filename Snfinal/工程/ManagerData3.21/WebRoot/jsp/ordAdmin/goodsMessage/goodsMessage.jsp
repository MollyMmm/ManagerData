...
switch(id)
			...
			location = "<%=path%>/GoodsMessageShowServlet?currentPage="+currentPage.value;
...
function Other(){
	...
	location = "<%=path%>/GoodsMessageShowServlet?currentPage="+goPage;
	}
}	
function altRows(id) {
...
window.onload = function() {
	...
	<body>
		<div style="margin-right: 10px; width: 850px">
			<form action="<%=path%>/GoodsMessageSearchServlet" method="post">
				<table>
					<tr>
					<td>
						...
						<td>
							...
							<a href="jsp/ordAdmin/goodsMessage/addGoodsMessage.jsp"><input
				...
				<td align="center" valign="middle">
					...
					<%
							if (list.size() == 0) {
								...
								<%if(request.getAttribute("leixing2") != null) {%>商品名：<%=request.getAttribute("leixing2")%><%}%>
								<%if(request.getAttribute("leixing3") != null) {%>商品描述：<%=request.getAttribute("leixing3")%><%}%>
								信息不存在，如需创建，请点击创建按钮！
								</td>
								</tr>
						...
						<%
							for (int i = 0; i < list.size(); i++) {
								Map<String, Object> GoodsMessage = list.get(i);
						%><tr>

							<td><%=GoodsMessage.get("ID")%></td>
							<td><%=GoodsMessage.get("EXTENDEDID")%></td>
							<td><%=GoodsMessage.get("NAMES")%></td>
							<td><%=GoodsMessage.get("PICTURE")%></td>
							<td><%=GoodsMessage.get("PRICE")%></td>
							<td><%=GoodsMessage.get("PREPRICE")%></td>
							<td><%=GoodsMessage.get("INTRODUCTION")%></td>
							<td>
								<a
									href="<%=path%>/GoodsMessageSelOneServlet?id=<%=GoodsMessage.get("ID")%>">编辑</a>|
								<a
									href="<%=path%>/GoodsMessageDelServlet?ID=<%=GoodsMessage.get("ID")%>">删除</a>
							</td>
						</tr>
						<%
							}
