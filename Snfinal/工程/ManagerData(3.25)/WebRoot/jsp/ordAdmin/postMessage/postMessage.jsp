...
<%
	...
	PageBean pageBean = (PageBean) request.getAttribute("postmessage");
...
switch(id)
			...
			location = "<%=path%>/PostMessageServlet?currentPage="+currentPage.value;
...
function Other(){
	...
	location = "<%=path%>/PostMessageServlet?currentPage="+goPage;
	}
}	
function altRows(id) {
...
function deleteDialog(id){
	...
	throughBox({
		...
		button: [
        {
            name: '确定',
            callback: function () {
               window.location.href = "<%=path%>/PostMessageDeleteServletID='+id";
            },
            focus: true
        },
        {
	...
	<body>
		<div style="margin-right: 10px; width: 850px">
			<form action="<%=path%>/PostMessageSearchServlet" method="post">
				<table>
					<tr>
					<td>
						...
						<td>
							...
							<a href="<%=path%>/jsp/ordAdmin/postMessage/addMessage.jsp"><input
				...
				<td align="center" valign="middle">
					...
					<%
							if (list.size() == 0) {
								...
								<%if(request.getAttribute("leixing3") != null) {%>内容：<%=request.getAttribute("leixing3")%><%}%>
								信息不存在，如需创建，请点击创建按钮！
								</td>
								</tr>
						...
						%><tr>
							...
							<td><%=message.get("USERID")%></td>
							<td><%=message.get("TITLE")%></td>
							<td><%=message.get("BODYS")%></td>
							<td><%=message.get("SENTTIME")%></td>

							<td>
								<a
									href="<%=path%>/PostMessageSelOneServlet?id=<%=message.get("ID")%>">编辑</a>|
								<a href="javascript:void(0)" id="<%=message.get("ID")%>" onclick="deleteDialog(this.id)"
									>删除</a>
							</td>
