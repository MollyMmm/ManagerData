...
public class NoticeServiceImpl implements Notice{
	...
	public boolean addNotice(NoticeBean notice,HttpSession session) {
		...
		String sql="insert into Notice(id,ExtendedId,Title,Body,MainBody,PublishTime,UpdateTime,Picture,Type,editor) values(Notice_seq.nextval,"+ExtendedId+",'"+Title+"','"+Body+"','"+MainBody+"','"+nowTime+"','"+nowTime+"','"+Picture+"',"+Type+",'@"+adminName+"')";
 ...
 */
	public boolean updateNotice(NoticeBean notice, HttpSession session) {
		...
		//获取原有的发布者
		String oldSql = "select editor from Notice where id="+String.valueOf(id);
		String oldEditor = (String)jdbc.query(oldSql).get(0).get("EDITOR");
		...
		String sql="update Notice set ExtendedId="+ExtendedId+",Title='"+Title+"',Body='"+Body+"',MainBody='"+MainBody+"',UpdateTime='"+UpdateTime+"',Picture='"+Picture+"',Type="+Type+",editor='"+newEditor+"' where id="+String.valueOf(id);
		System.out.println("-------------"+sql);		
		int operator = 3;
		return dao.update(sql, operator, session);
	}
...
System.out.println("------"+sql);
	...
	public String selectId(String title,String body){
		String sql = "select id from notice where title='"+title+"' and body ='"+body+"'";
